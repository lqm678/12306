...
STATION_DATES = Config['set']['station_dates']

# 填入需要购买的车次(list)，"G1353"
# 修改车次填入规则，注：(以前设置的车次逻辑不变)，如果车次填入为空，那么就是当日乘车所有车次都纳入筛选返回
# 不填车次是整个list为空才算，如果不是为空，依然会判断车次的，这种是错误的写法 [""], 正确的写法 []
STATION_TRAINS = Config['set']['station_trains']

# 出发城市，比如深圳北，就填深圳就搜得到
FROM_STATION = Config['set']['from_station']

# 到达城市 比如深圳北，就填深圳就搜得到
TO_STATION = Config['set']['to_station']
...
# "无座",
# "动卧",
SET_TYPE = Args.set_type if Args.set_type != '' else (
    Config['set']['set_type'] if str(Config['set']['set_type']) != '' else ['软卧', '二等座'])

# 当余票小于乘车人，如果选择优先提交，则删减联系人和余票数一致在提交
# bool
IS_MORE_TICKET = Config['set']['is_more_ticket']
...
PWD = Args.Pass if Args.Pass != '' else Config['set']['12306account'][1]['pwd']
...
ORDER_MODEL = Args.order_model if Args.order_model != '' else (
    Config['order_model'] if str(Config['order_model']) != '' else 1)
...
sleepIntervals = re.split(
    '\s*,\s*', Args.sleep_intervals) if (re.match(r'\S+', Args.sleep_intervals)) else []
